  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:3000/api/listing/update/${params.listingId}`, {
        ...
        credentials: "include", // Add this line to include cookies (authentication token)
        body: JSON.stringify({
          ...formData,
      ...
      // Make sure data._id exists before navigating
      if (data._id) {
        navigate(`/listing/${data._id}`);
      } else {
        setError("Update successful but listing ID was not returned. Please go to your profile to see listings.");
      }
    } catch (error) {
