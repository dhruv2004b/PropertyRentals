...
export default function CreateListing() {
    ...
    const handleImageSubmit= (e)=>{
        if(files.length>0 && files.length + formData.imageUrls.length <7){
            ...
            Promise.all(promises).then((urls)=>{
                ...
                setImageUploadError('Image upload Failed (Maximum size 2 MB per Image)');
                setUploading(false);
            });
        }else{
  ...
  return (
    <main className='p-3 max-w-4xl mx-auto'>
      <h1 className='text-3xl font-semibold text-center my-7'>Create Your Listing</h1>
      <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4' action="">
        <div className="flex flex-col gap-4 flex-1">
            ...
            <textarea onChange={handleChange} value={formData.description} type="text" placeholder='Description' className='border p-3 rounded-lg' id='description' maxLength='62' minLength='10' required />
            ...
            <div className="flex flex-wrap gap-6">
                ...
                <div className="flex items-center gap-2">
                    <input onChange={handleChange} value={formData.regularPrice} className='p-3 border border-x-gray-300 rounded-lg' type="number" id='regularPrice' min='1' max='10000000' required />
        ...
        <button disabled={loading || uploading} className='p-3 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-80'>{loading?'Listing ...':'Create Listing'}</button>
